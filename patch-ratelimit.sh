#!/bin/bash
# ============================================
# Generate ratelimit.go dengan isLoadTestRequest
# Jalankan SEKALI saja
# ============================================
set -e

TARGET="/c/C/projects/cbt-quasar/cbt/backend/internal/middleware/ratelimit.go"

if [ ! -f "$TARGET" ]; then
  echo "❌ File tidak ditemukan: $TARGET"
  exit 1
fi
...
echo "✅ File ratelimit.go berhasil di-update"
echo ""
echo "Verify:"
grep -n "isLoadTestRequest" "$TARGET" | head -3
