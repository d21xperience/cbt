  ...
  <q-page class="flex flex-center bg-blue-grey-10">
    <q-card style="width: 400px; max-width: 90vw;" class="shadow-24 rounded-borders">
      ...
      <q-card-section class="bg-grey-9 text-white text-center q-pa-lg">
        ...
        <div class="text-caption text-grey-4 text-uppercase text-weight-light">SaaS Central Infrastructure</div>
      </q-card-section>

      <!-- Form Pengisian Kredensial -->
      <q-card-section class="q-pt-xl q-px-lg">
        <q-form @submit.prevent="onSuperLogin" class="q-gutter-md">
          <q-input v-model="username" label="Root Username" outlined dark bg-color="grey-9"
            :rules="[val => !!val || 'Kredensial root wajib diisi']">
            <template v-slot:prepend><q-icon name="shield" color="primary" /></template>
          </q-input>

          <PasswordInput v-model="password" label="Master Password" dark bg-color="grey-9"
            :rules="[val => !!val || 'Kunci master wajib diisi']" class="q-mb-lg" />

          <div class="q-pt-sm">
            <q-btn label="Aktivasi Sesi Sistem" type="submit" color="primary" class="full-width text-weight-bold"
