        ...
        <q-card>
          <q-card-section>
            ...
            <q-form @submit.prevent="onSync" class="q-gutter-md">
              <q-select
                v-model="form.pembelajaran_id"
                :options="pembelajaranOptions"
                label="Pembelajaran"
                outlined
                emit-value
                map-options
                :rules="[(val) => !!val || 'Pembelajaran wajib dipilih']"
              >
              ...
              <q-banner class="bg-blue-1 text-blue-9 q-mb-md" rounded>
                ...
                Sinkronisasi akan menarik data peserta dari SIAKAD (Dapodik) berdasarkan
                pembelajaran dan semester yang dipilih. Data yang sudah ada akan di-update.
              </q-banner>
              ...
              <q-btn
                ...
                icon="sync"
                class="full-width"
                :loading="adminStore.isSyncing"
                :disable="adminStore.isSyncing"
              />
            </q-form>
          </q-card-section>
        </q-card>
              ...
              <q-item>
                ...
                <q-item-section>
                  ...
                  <q-item-label caption
                    >Jangan tutup halaman saat proses sync berlangsung</q-item-label
                  >
                </q-item-section>
              </q-item>
            </q-list>
        ...
        <q-card class="q-mt-md" v-if="lastSyncResult">
          <q-card-section>
            ...
            <div class="text-center">
              <q-icon
                :name="lastSyncResult.success ? 'check_circle' : 'error'"
                :color="lastSyncResult.success ? 'positive' : 'negative'"
                size="60px"
              />
              <div
                class="text-h5 q-mt-md"
                :class="lastSyncResult.success ? 'text-positive' : 'text-negative'"
              >
...
const onSync = async () => {
  $q.dialog({
    ...
    try {
      const result = await adminStore.syncFromSiakad(form.pembelajaran_id, form.semester_id)
