    ...
    <q-card>
      ...
      >
        ...
        <template v-slot:body-cell-session_type="props">
          <q-td :props="props">
            <q-badge
              :color="props.row.session_type === 'REGULER' ? 'primary' : 'orange'"
              :label="props.row.session_type"
              outline
            />
          </q-td>
        </template>
        ...
        <q-card-section>
          <q-form @submit.prevent="onCreateSession" class="q-gutter-md">
            ...
            <q-select
              ...
              emit-value
              map-options
              :rules="[(val) => !!val || 'Tipe sesi wajib dipilih']"
            />
...
const examOptions = computed(() => adminStore.exams.map((e) => ({ label: e.name, value: e.id })))
...
onMounted(async () => {
  loading.value = true
  try {
    await Promise.all([adminStore.fetchSessions(), adminStore.fetchExams()])
  } catch {
