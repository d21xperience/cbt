<template>
  <q-page padding>
    ...
    <q-table :rows="store.schools" :columns="columns" row-key="id" flat bordered>
      ...
      <template v-slot:body-cell-sync="props">
        <q-td :props="props" class="text-center">
          <q-btn
            dense
            :color="props.row.sync_locked ? 'red-5' : 'green-5'"
            :icon="props.row.sync_locked ? 'lock' : 'lock_open'"
            :label="props.row.sync_locked ? 'Locked' : 'Allowed'"
      ...
      <template v-slot:body-cell-actions="props">
        <q-td :props="props" class="q-gutter-xs text-center">
          ...
          <q-btn
            ...
            color="indigo-9"
            icon="settings"
            @click="openSaasConfig(props.row.id)"
          >
          ...
          <q-btn
            ...
            color="negative"
            icon="block"
            @click="suspendSchool(props.row.id)"
          />
        </q-td>
      </template>
    </q-table>
    ...
    <school-form-dialog
      v-model="dialogOpen"
      :edit-data="selectedSchool"
      @saved="store.getSchoolsAction()"
    />

    <!-- Dialog Interaktif Konfigurasi SaaS Tingkat Lanjut -->
    <saas-config-dialog
      v-model="saasDialogOpen"
      :school-id="selectedSchoolId"
      @success="store.getSchoolsAction()"
    />
  </q-page>
</template>
...
const suspendSchool = (schoolId) => {
  $q.dialog({
    ...
    message:
      'Apakah Anda yakin ingin menonaktifkan seluruh akses ujian sekolah ini sementara waktu?',
    cancel: true,
    persistent: true,
  }).onOk(() => {
