  ...
  <q-dialog v-model="isOpen" persistent>
    <q-card style="width: 550px; max-width: 90vw;">
      ...
      <q-card-section class="q-gutter-md q-pt-md">
        ...
        <!-- Input Subdomain -->
        <q-input v-model="form.subdomain" label="Subdomain CBT" outlined dense prefix="https://"
          suffix=".ulangan.co.id">
          <template v-slot:hint>
            Alamat akses unik untuk sekolah ini.
          </template>
        </q-input>
        ...
        <div class="row q-col-gutter-sm">
          <div class="col-12 col-sm-6">
            <q-select v-model="form.package" :options="packageOptions" label="Paket Sewa" outlined dense emit-value
              map-options />
          </div>
          <div class="col-12 col-sm-6">
            <q-input v-model.number="form.max_participants" type="number" label="Kuota User Aktif" outlined dense />
          </div>
        </div>
      </q-card-section>
      ...
      <q-card-actions align="right" class="text-primary q-pb-md q-pr-md">
        ...
        <q-btn color="primary" label="Daftarkan & Aktivasi" @click="submitForm" :loading="submitting" />
      </q-card-actions>
    </q-card>
  </q-dialog>
</template>
...
const form = ref({ npsn: '', nama: '', subdomain: '', package: 'monthly_500', max_participants: 500 })
