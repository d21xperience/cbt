...
<template>
  <q-page class="flex flex-center bg-grey-1">
    <div class="main-container q-pa-md">
      ...
      <q-input v-model="searchQuery" outlined dense placeholder="Cari sekolah (nama, NPSN, atau subdomain)..."
        class="q-mb-lg" clearable :loading="loading">
        ...
        <div class="text-grey-6 q-mt-sm">
          {{
            searchQuery
              ...
              : 'Belum ada sekolah yang bergabung di platform ini.'
          }}
        </div>
      </div>

      <div v-else class="row q-col-gutter-md">
        <div v-for="school in filteredSchools" :key="school.subdomain" class="col-12 col-sm-6 col-md-4">
          <q-card flat bordered class="bg-indigo-50-hover cursor-pointer" @click="redirectToTenant(school.subdomain)">
            <q-card-section>
              <div class="flex items-center">
                <q-avatar size="40px" class="q-mr-sm">
                  <img :src="school.logo_url || 'https://cdn.quasar.dev/logo/svg/quasar-logo.svg'" alt="Logo" />
                </q-avatar>
                <div>
...
onMounted(() => {
  // Hydrate tenant dari ?tenant= kalau ada
  tenantStore.hydrateFromUrl()

  // Notif kalau datang dari guard
  if (route.query.needTenant === '1') {
    $q.notify({
      type: 'warning',
      icon: 'warning',
      message: 'Silakan pilih sekolah terlebih dahulu untuk melanjutkan.',
      position: 'top',
      timeout: 4000,
    })
    // Bersihkan query biar tidak muncul terus
