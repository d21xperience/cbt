...
export const participantHandlers = (mock) => {
    ...
    return new Promise((resolve) => {
      ...
      Papa.parse(csvFile, {
        ...
        complete: (results) => {
          ...
          resolve([200, { preview, errors: preview.filter(p => p._error).map(p => ({ row: p._rowNumber, message: p._error })) }]);
        },
        error: (error) => {
  ...
  }, { delay: 1500 }); // delay bisa ditambahkan di opsi
  ...
  mock.onPost('/admin/participants/import-external/confirm').reply((config) => {
    ...
    return [200, { message: 'Peserta berhasil diimport', imported_count: newParticipants.length }, { delay: DELAY }];
  });
